...
import { CheckoutMessages } from '../data/messages';
...
export class CheckoutPage extends BasePage {
  ...
  async expectItemDetailsInOrder(name: string, price: string) {
    const actualName = await this.page.locator('td.product-name').evaluate(el =>
      el.childNodes[0].textContent?.trim()
    );
    const actualPrice = await this.page.locator('td.product-total').first().innerText();
    expect(actualName).toMatch(new RegExp(name, 'i'));
    expect(actualPrice).toBe(price);
  }

  async fillBillingDetails(info: typeof checkoutInfo) {
    ...
    (await this.stateSelect.isVisible()) && await this.stateSelect.selectOption({ label: info.state });
    (await this.stateInput.isVisible()) && await this.stateInput.fill(info.state);
  ...
  async placeOrder(info: typeof checkoutInfo) {
  ...
  async expectOrderPageDisplayed() {
    await expect(this.page.getByText(CheckoutMessages.ORDER_SUCCESS)).toBeVisible();
    await expect(this.page).toHaveURL(/\/order-received\/?/);
  }

  async expectOrderInfoDisplayed(info: typeof checkoutInfo, name: string, price: string) {
    const billingAddress = await this.page.locator('address').innerText();
    const productName = await this.page.locator('td.product-name a').innerText();
    const productTotal = await this.page.locator('td.product-total').innerText();
